                    ...
                    stream.readPending(
                            message -> {
                                ...
                                if (message.isTransactionalMessage()) {
                                    if (!connectorConfig.shouldProvideTransactionMetadata()) {
                                        ...
                                        if (message.getOperation()
                                                == ReplicationMessage.Operation.COMMIT) {
                                ...
                                // DML event
                                    ...
                                    // 如果是__deCycle_table表的消息，则记录事务ID，并跳过该消息
                                    if (message.getTable() != null
                                            && "\"public\".\"__decycle_table\""
                                                    .equals(message.getTable())) {
                                        LOGGER.info(
                                                "Received message from __deCycle_table, skip it");
                                        this.skipTransactionId = message.getTransactionId();
                                        return;
                                    }

                                    // Skip messages with transaction id that has been skipped
                                    // before
                                    if (skipTransactionId.equals(message.getTransactionId())) {
                                        LOGGER.info(
                                                "Skip message with transaction id: {}",
                                                message.getTransactionId());
                                        return;
                                    }
                                    TableId tableId = null;
                                    if (message.getOperation()
                                            != ReplicationMessage.Operation.NOOP) {
